...
def create_maia_addons_config( form,
                              cluster_config_file,
                             ):
    ...
    metallb_shared_ip = cluster_config["metallb_shared_ip"] if "metallb_shared_ip" in cluster_config else None
    ...
    with open(f"{namespace}_maia_addons.tf.json", "w") as f:
        json.dump(maia_addons_template, f, indent=2)

    with open(f"{namespace}_maia_addons_values.yaml", "w") as f:
    ...
    print(f"helm repo add maiakubegate {helm_repo}")
    print(
        f"helm upgrade --install -n {helm_namespace} maia-addons maiakubegate/{helm_chart} --values {namespace}_maia_addons_values.yaml --version={helm_repo_version}")
