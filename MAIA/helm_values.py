...
def read_config_dict_and_generate_helm_values_dict(config_dict: Dict[str, Any], kubeconfig_dict: Dict[str, Any]) -> \
    ...
    if 'existing_persistent_volume' in config_dict:
        for idx, persistent_volume in enumerate(config_dict['existing_persistent_volume']):
            ...
            for persistent_volume_key in persistent_volume:
                value_dict["existingPersistentVolume"][-1][persistent_volume_key] = \
                    config_dict['existing_persistent_volume'][idx][
                        persistent_volume_key]
            ...
            for user_id, user in enumerate(config_dict['user_secret']):
                for idx, user_secret_param in enumerate(config_dict['user_secret_params']):
                    value_dict["extraEnv"].append({
                        ...
                        "valueFrom": {
                            "secretKeyRef": {"key": user_secret_param, "name": user}},
    ...
    if 'env_variables' in config_dict:
        ...
        for env_variable in config_dict["env_variables"]:
            value_dict["extraEnv"].append({
                "name": env_variable,
                "value": config_dict['env_variables'][env_variable]
        ...
        for idx, port in enumerate(config_dict['ports']):
            if idx == 0:
                value_dict["service"]["ports"] = [
                    {
                        "port": config_dict['ports'][port][0],
                        "targetPort": config_dict['ports'][port][0],
                        "name": port
                    }]
